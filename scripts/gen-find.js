...
const NEEDS = [
  ...
  },
  {
    key: 'kids',
    label: '소아 진료',
    question: '어린이 진료가 가능한 치과',
    detect: p => /소아/.test(`${(p.specialties ?? []).join(' ')} ${(p.features ?? []).join(' ')}`),
    tip: '소아 진료는 아이의 연령·협조도에 따라 진료 방식이 달라질 수 있으므로, 예약 시 아이 나이와 상태를 미리 알려주면 진료가 수월합니다.',
  },
  {
    key: 'sedation',
    label: '진정(수면)치료',
    question: '의식하진정(수면) 방식 치료가 가능한 치과',
    detect: p => /의식하진정|수면치료|진정치료|수면\s?진정/.test(`${(p.specialties ?? []).join(' ')} ${(p.features ?? []).join(' ')}`),
    tip: '진정(수면)치료는 사전 검사와 금식 등 준비가 필요할 수 있어 반드시 예약 시 상담이 필요합니다. 치과 공포가 큰 분들이 주로 이용합니다.',
  },
];
